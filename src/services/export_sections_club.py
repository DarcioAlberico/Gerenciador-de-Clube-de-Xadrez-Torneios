...
class ClubSectionsMixin:
    def _club_report_sections(self) -> list[tuple[str, list[str], list[list[Any]]]]:
        ...
        club_rows = [
            [
                ...
                club["name"],
                club.get("kind", ""),
                "Sim" if club.get("active") else "Não",
                club.get("city", ""),
                club.get("phone", ""),
                club.get("email", ""),
        ...
        guardian_rows = [
            [
                ...
                guardian["email"],
                guardian["document"],
                "Sim" if guardian.get("active") else "Não",
        ...
        plan_rows = [
            [
                ...
                "Sim" if plan.get("active") else "Não",
                plan["payments_count"],
            ]
            for plan in plans
        ]
        ...
        indicator_rows = [
            ...
            ["Menores sem responsável", len(minor_members_without_guardians)],
    ...
    def _tournaments_period_sections(
        ...
        return [
            ("Período", ["Campo", "Valor"], period_rows),
            (
                "Torneios por período",
                [
                    "ID",
                    "Nome",
                    "Escopo",
    ...
    def _attendance_report_sections(
        ...
        summary_rows: list[list[Any]] = [
            ["Data inicial", start_date or "Sem filtro"],
            ["Data final", end_date or "Sem filtro"],
            ["Aulas/treinos no período", len(sessions)],
            ["Registros de chamada", len(rows)],
            ["Presenças", sum(1 for item in rows if item["status"] == "present")],
    ...
    def _financial_report_sections(
        ...
        return [
            ...
            (
                "Planos",
                ["ID", "Nome", "Valor", "Ciclo", "Ativo", "Lancamentos", "Observações"],
                plan_rows,
            ),
            (
                "Lancamentos",
    ...
    def _internal_ranking_sections(
        ...
        return [
            ...
            (
                "Categorias",
                ["Categoria", "Membros", "Líder", "Rating líder", "Aproveitamento líder"],
                category_rows,
            ),
        ]
    ...
    def _administrative_package_sections(
        ...
        cover_rows = [
            ["Gerado em", Database.now()],
            ["Período inicial", start_date or "Sem filtro"],
            ["Período final", end_date or "Sem filtro"],
    ...
    def _member_evolution_sections(
        ...
        if not member:
            raise AppError("Membro não encontrado.")
        ...
        member_rows: list[list[Any]] = [
            ...
            ["Tags premiação", member.get("prize_tags", "")],
            ["Cidade", member["city"]],
        ...
        guardian_rows: list[list[Any]] = [
            [
                item["guardian_name"],
                item["relationship"],
                "Sim" if item.get("primary_contact") else "Não",
                "Sim" if item.get("emergency_contact") else "Não",
                item["guardian_phone"],
                item["guardian_email"],
                item["guardian_document"],
                "Sim" if item.get("guardian_active") else "Não",
                item["notes"],
            ]
        ...
        attendance_summary_rows: list[list[Any]] = [
            ...
            ["Presenças", attendance_summary["present"]],
            ["Faltas", attendance_summary["absent"]],
            ["Faltas justificadas", attendance_summary["justified"]],
            ["Frequência", f"{attendance_summary['attendance_rate']}%"],
        ]
        ...
        return [
            ...
            ("Resumo de frequência", ["Indicador", "Valor"], attendance_summary_rows),
            (
                "Presenças",
                [
                    "Data",
