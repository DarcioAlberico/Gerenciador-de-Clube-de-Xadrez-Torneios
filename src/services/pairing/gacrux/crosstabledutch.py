...
class crosstable_dutch(crosstable):
        ...
        if update_maxpsd:
            ...
            if maxpsd != self.maxpsd:
                ...
                raise AssertionError(
                    f"Gacrux crosstabledutch: maxpsd inconsistente: "
                    f"calculado={maxpsd}, esperado={self.maxpsd}"
                )
    ...
    def update_weight(self, mode, category, c):
        ...
        if mode == "QC":
            ...
            # Era breakpoint() — substituído para não bloquear modo não-interativo.
            raise ValueError(
                f"Gacrux crosstabledutch.update_weight: modo desconhecido '{mode}'. "
                f"Modos válidos: QC, HE, HO, BI."
            )
        c["mode"] = mode
        c["levels"] = category
        c["weight"] = weight
